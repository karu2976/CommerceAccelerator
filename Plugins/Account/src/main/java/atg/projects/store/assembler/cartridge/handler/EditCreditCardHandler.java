...
/**
 * Cartridge handler for editing cards. Retrieves the card to edit from the current request
 * then returns its properties in the ContentItem. The list of countries is always added to the
 * content item, along with the list of states (code and display name), card nickname and
 * whether or not this is the default card.
 *
 ...
 * "cardNickname": "Visa"
 *
 * The states entry is a list of states. The counties entry is a list of countries. The
 * billingAddress contains the billing address properties.
 *
 * @author Oracle
 * @version $Id: //hosting-blueprint/CSA/version/11.3/Plugins/Account/src/main/java/atg/projects/store/assembler/cartridge/handler/EditCreditCardHandler.java#1 $$Change: 1385662 $
 * @updated $DateTime: 2017/03/09 10:29:42 $
 */
public class EditCreditCardHandler extends NavigationCartridgeHandler{

  /** Class version string */
  public static String CLASS_VERSION = "$Id: //hosting-blueprint/CSA/version/11.3/Plugins/Account/src/main/java/atg/projects/store/assembler/cartridge/handler/EditCreditCardHandler.java#1 $$Change: 1385662 $";
  ...
  /** Key in the content item for the list of supported countries */
  ...
  // property: yearList
  //-----------------------------------
  private YearListGenerator mYearList;

  /**
   * @return A component used to generate a list of years.
   */
  ...
  /**
   * @param pYearList A component used to generate a list of years.
   */
  ...
  /**
   * Gets the card to edit from the getProfilePropertyManager().getCreditCardNicknamePropertyName()
   * property on the request. Returns its properties as specified by filterId, along with countries,
   * billingAddress.states and nickname.
   *
   * @param pContentItem A content item.
   * @return A content item containing page names and URLs.
  ...
  public ContentItem process(ContentItem pContentItem) throws CartridgeHandlerException {
    ...
    if(currRequest != null) {
      String nickname =
        currRequest.getParameter(getProfilePropertyManager().getCreditCardNicknamePropertyName());
      if(!StringUtils.isEmpty(nickname)){

        Profile currProfile = (Profile) ServletUtil.getCurrentUserProfile();
        try {
          Map<String, Object> filtered = getProfileTools().getFilteredCardMap(currProfile, nickname,
